...
export default function NotLoggedInPage({ handleStatusChange, status }) {
  ...
  return (
    <Wrapper>
      ...
      <ButtonContainer>
        <ChoiceButton status={!registrationOption} onClick={() => setRegistrationOption(false)}>
          Log In
        </ChoiceButton>
        <ChoiceButton status={registrationOption} onClick={() => setRegistrationOption(true)}>
          Register
        </ChoiceButton>
      </ButtonContainer>
      <ContentContainer>
        {!registrationOption ? (
          <LoginForm handleStatusChange={handleStatusChange} status={status} />
        ) : (
          <RegistrationForm setRegistrationOption={setRegistrationOption} />
        )}
      </ContentContainer>
    </Wrapper>
  )
