...
test('No logged in user found -> returns input with pushed status', () => {
    expect(calcUserScore(userNotLoggedIn))
    .toEqual([...userNotLoggedIn, {user: false, custom: false}])
})

test('User logged in but no custom weights -> returns input with pushed status', () => {
    expect(calcUserScore(userLoggedInNoCustom))
    .toEqual([...userLoggedInNoCustom, {user: true, custom: false}])
})

test('User logged in with custom weights -> returns input with modified country data', () => {
    expect(calcUserScore(userLoggedInCustom))
    .toEqual([ 
